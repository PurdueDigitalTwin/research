...
class PPOModel(_model.Model):
    ...
    def forward( # type: ignore
        ...
        **kwargs,
        ...
        mean, log_std, values = typing.cast(typing.Tuple[jax.Array, jax.Array, 
                                                         jax.Array], raw_outputs)
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        assert action.shape == mean.shape, f"Expected action shape {mean.shape}, \
        ...
        var = std ** 2
        log_act_probs = -0.5 * (jnp.log(2 * jnp.pi) + 2 * log_std + \
                            ((action - mean) ** 2) / var)
