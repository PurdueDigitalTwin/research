...
class PPOModel(_model.Model):
    ...
    def __init__(
        ...
        self._network = policy.ContinuousActorCriticPolicy(
            features=64,
    ...
    @property
    @typing_extensions.override
    def network(self) -> policy.ContinuousActorCriticPolicy:
    ...
    def forward(  # type: ignore
        ...
        **kwargs,
        ...
        Returns:
            mean (jax.Array): Mean of the action distribution, with shape
                `(*, action_space_dim)`.
            log_std (jax.Array): Log standard deviation of the action distribution,
                with shape `(*, action_space_dim)`.
            values (jax.Array): State value estimates, with shape `(*,)`.
        """
        del rngs, kwargs

        raw_outputs = self._network.apply(params, inputs=state)

        mean, log_std, values = typing.cast(
            typing.Tuple[jax.Array, jax.Array, jax.Array], raw_outputs
        )
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        raw_outputs = self._network.apply(params, rngs=rngs, inputs=state)
        ...
        # assert action shape is (*, action_space_dim)
        assert (
            action.shape == mean.shape
        ), f"Expected action shape {mean.shape}, \
            but got {action.shape}."
        ...
        var = std**2
        log_act_probs = -0.5 * (
            jnp.log(2 * jnp.pi) + 2 * log_std + ((action - mean) ** 2) / var
        )
        log_act_probs = log_act_probs.sum(axis=-1)
        ...
        surrogate_loss = jnp.minimum(
            ratios * advantages,
            jnp.clip(ratios, 1.0 - clip_epsilon, 1.0 + clip_epsilon)
            * advantages,
        )
        ...
        # entropy_loss = self._entropy_coeff * (-jnp.mean(entropy))
        entropy_loss = 0.0
